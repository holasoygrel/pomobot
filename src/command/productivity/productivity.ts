...
import { getUserMinutesStudied, userExists } from "../../databases/resolvers/UserResolver";
...
let productivityMessage = (minutesStudied: string): string => {
    return `\`\`\`tiempo total trabajado: ${minutesStudied}\`\`\``;
}

export let minutesToHours = (totalMinutes: number): string => {
    let hours = Math.floor(totalMinutes / 60);          
    let minutes = totalMinutes % 60;
    let hourSingularOrPlural = (hours === 1) ? 'hora' : 'hora';
    return (hours === 0) ? `${minutes} minutos`: `${hours} ${hourSingularOrPlural} ${minutes} minutos`;
}